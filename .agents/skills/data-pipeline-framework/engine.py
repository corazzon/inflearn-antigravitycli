...
이 스크립트는 config.json 설정 파일을 입력받아,
신규 프로젝트 디렉토리 생성, 템플릿 코드 배포/치환 및
수집-분석-대시보드 가동으로 구성되는 데이터 파이프라인 워크플로우를 오케스트레이션합니다.

주요 명령어:
- init: 프로젝트 폴더 및 소스코드 구성
- run: 전체 데이터 파이프라인 또는 개별 단계 실행
...
def run_cmd(command, cwd=None):
    ...
    if res.returncode != 0:
        print(f"[Engine] [Error] 명령어 실행 실패 (반환 코드: {res.returncode}): {command}")
        return False
    return True
...
def cmd_init(args):
    ...
    replacements = {
        ...
        "__MAX_RETRIES__": str(config.get("retry_config", {}).get("max_retries", 5)),
        "__BACKOFF_FACTOR__": str(config.get("retry_config", {}).get("backoff_factor", 1.5)),
...
def cmd_run(args):
    ...
    step = args.step
    print(f"[Engine] 파이프라인 단계 구동 시작: {step} (프로젝트: {project_name})")
    ...
    if step == "all" or step == "scrape":
        ...
        if hook:
            ...
            if not run_cmd(hook):
                print("[Engine] [Warning] post_scrape 훅 실행 실패. 후속 단계를 계속 진행합니다.")
    ...
    if step == "all" or step == "dashboard":
        print("[Engine] [Step 4] 대시보드 데이터 빌드 (dashboard_data_builder.py) 실행...")
        if not run_cmd(f"{python_exe} {project_name}/src/dashboard_data_builder.py"):
            sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(description="Antigravity 데이터 파이프라인 프레임워크 CLI 엔진")
    subparsers = parser.add_subparsers(dest="command", required=True)
    
    # init command
    parser_init = subparsers.add_parser("init", help="신규 프로젝트 디렉토리 생성 및 소스코드 복사/치환")
    parser_init.add_argument("--config", required=True, help="설정 파일(config.json) 경로")
    parser_init.set_defaults(func=cmd_init)
    
    # run command
    parser_run = subparsers.add_parser("run", help="데이터 파이프라인 단계별 구동")
    parser_run.add_argument("--config", required=True, help="설정 파일(config.json) 경로")
    parser_run.add_argument("--step", choices=["all", "inspect", "scrape", "eda", "dashboard"], default="all", help="구동할 단계 (기본값: all)")
